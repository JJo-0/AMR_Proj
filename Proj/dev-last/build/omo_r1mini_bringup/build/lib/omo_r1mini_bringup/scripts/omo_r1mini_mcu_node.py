...
class OMOR1MiniNode(Node):
  def __init__(self):
    ...
    self.wheel_radius = 0.0965
    self.enc_pulse = 240.0
    ...
    self.distance_per_pulse = 2*math.pi*self.wheel_radius / self.enc_pulse
    ...
    # Set subscriber
    self.subLeftEnc = self.create_subscription(Int32, 'l_enc', self.RsvLEnc, 10)
    self.subRightEnc = self.create_subscription(Int32, 'r_enc', self.RsvREnc, 10)
  ...
  def RsvLEnc(self, msg):
    self.l_enc = msg.data
  def RsvREnc(self, msg):
    self.r_enc = msg.data
  def RsvImu(self, msg):
  ...
  def update_robot(self):
    odo_l = self.distance_per_pulse * self.l_enc
    odo_r = self.distance_per_pulse * self.r_enc
    trans_vel = (odo_r + odo_l) / 2.0
