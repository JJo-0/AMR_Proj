...
class ControlPanel(QWidget):
        ...
        if data.startswith("E_"):
            try:
                ...
                if status == 1:
                    ...
                    self.emergency_stop_button.setChecked(False)  # 추가된 부분
                elif status == 0:
                    self.emergency_pub.publish(Int32(data=0))
                    self.update_status_label("EMS Signal", "Emergency: 0", "red")
                    self.emergency_stop_button.setChecked(True)  # 추가된 부분
    ...
    def start_lift(self, command, log_message):
        ...
        self.log_to_terminal("Lift Timer Started")
    ...
    def start_movement(self, direction):
        ...
        self.movement_timer.timeout.connect(lambda: self.send_movement_command(direction))
